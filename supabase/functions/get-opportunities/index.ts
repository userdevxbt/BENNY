// @ts-nocheck - Legacy codebase compatibility
...
serve(async (req) => {
  ...
  try {
    ...
    const { isActive = true, limit = 500 } = body as {
      isActive?: boolean;
      limit?: number;
    };

    console.log(`📡 get-opportunities called: limit=${limit}, isActive=${isActive}`);

    // Simple single query - avoid complex batching for mobile
    const query = supabase
      .from("opportunities")
      .select("*")
      .eq("is_active", isActive)
      .order("created_at", { ascending: false })
      .limit(Math.min(limit, 1000)); // Max 1000 per query
    ...
    if (error) {
      ...
      const { data: rpcData, error: rpcError } = await supabase
        .rpc('get_active_opportunities', { p_limit: limit });
    ...
    const elapsed = Date.now() - startTime;
    console.log(`✅ Success: ${data?.length || 0} opportunities in ${elapsed}ms`);
